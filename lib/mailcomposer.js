...
var Stream = require("stream").Stream,
    ...
    runDKIMSign = require("./dkim").DKIMSign,
...
MailComposer.prototype._emitDataElement = function(element, callback){
    ...
    if(element.contents){
        ...
        if(!this._cacheOutput){
            this.emit("data", new Buffer(data.replace("\r\n", this._lineEnding) + this._lineEnding, "utf-8"));
        }else{
...
MailComposer.prototype._serveStream = function(stream, callback){
    ...
    stream.on("data", (function(chunk){
        ...
        data = buffer.slice(0, len - remainderLength).toString("base64").replace(/.{76}/g,"$&"+this._lineEnding);
    ...
    stream.on("end", (function(){
        ...
        if(remainder.length){
            data = remainder.toString("base64").replace(/.{76}/g,"$&"+this._lineEnding);
            if(!this._cacheOutput){
                this.emit("data", new Buffer(data.trim()+this._lineEnding, "utf-8"));
            }else{
                this._outputBuffer += data.trim()+this._lineEnding;
