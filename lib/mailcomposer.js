...
MailComposer.prototype._composeHeader = function() {
    ...
    if (!this._cacheOutput) {
        this.emit("data", new Buffer(headers.join(CRLF) + CRLF + CRLF, "utf-8"));
    } else {
        this._outputBuffer += headers.join(CRLF) + CRLF + CRLF;
    }
};
    ...
    for (var len = flatTree.length; this._message.processingPos < len; this._message.processingPos++) {
        ...
        if (isEnd || isObject) {
            ...
            if (slice && slice.length) {
                if (!this._cacheOutput) {
                    this.emit("data", new Buffer(slice.join(CRLF) + CRLF, "utf-8"));
                } else {
                    this._outputBuffer += slice.join(CRLF) + CRLF;
                }
            }
...
MailComposer.prototype._emitDataElement = function(element, callback) {
    ...
    if (element.contents) {
        ...
        if (!this._cacheOutput) {
            this.emit("data", new Buffer(data + CRLF, "utf-8"));
        } else {
            this._outputBuffer += data + CRLF;
        }
        ...
        if (err || !stat.isFile()) {
            ...
            if (!this._cacheOutput) {
                this.emit("data", new Buffer(new Buffer("<ERROR OPENING FILE>",
                    "utf-8").toString("base64") + CRLF, "utf-8"));
            } else {
                this._outputBuffer += new Buffer("<ERROR OPENING FILE>",
                    "utf-8").toString("base64") + CRLF;
            }
...
MailComposer.prototype._serveStream = function(stream, callback) {
    ...
    stream.on("data", (function(chunk) {
        ...
        if (data.length) {
            if (!this._cacheOutput) {
                this.emit("data", new Buffer(data.trim() + CRLF, "utf-8"));
            } else {
                this._outputBuffer += data.trim() + CRLF;
            }
        }
    }).bind(this));
...
MailComposer.prototype._processBufferedOutput = function() {
    ...
    if (this._dkim) {
        if ((dkimSignature = runDKIMSign(this.options.escapeSMTP ? this._outputBuffer.replace(/^\.\./mg, ".") : this._outputBuffer, this._dkim))) {
            this.emit("data", new Buffer(dkimSignature + CRLF, "utf-8"));
        }
    }
