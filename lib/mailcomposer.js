...
MailComposer.prototype.setMessageOption = function(options){
    var fields = ["from", "to", "cc", "bcc", "replyTo", "inReplyTo", "references", "subject", "body", "html", "envelope", "ical"],
...
MailComposer.prototype._generateBodyStructure = function(){
    ...
    if(this._message.ical){
        node = this._createiCalComponent(this._message.ical, "text/calendar; method=REQUEST ");
        if(currentNode){
            currentNode.childNodes.push(node);
            node.parentNode = currentNode;
        }else{
...
/**
 * <p>Creates a mime tree node for a ical component</p>
 *
 * @param {String} text 7bit (ascii) encoded text
 * @param {String} [contentType="text/calendar"] Content type for the text component
 * @return {Object} Mime tree node 
 */
MailComposer.prototype._createiCalComponent = function(text, contentType){
    var node = this._createMimeNode();
    
    node.contentEncoding = "7bit" //"quoted-printable";
    node.useTextType = true;
    ...
    if(Array.isArray(headers)){
        headersArr = headers.map((function(field){
            var key = this._normalizeKey(field.key || field[0]),
                value = this._sanitizeHeaderValue(field.value || field[1], key);
        ...
        for(var i=0, len = keys.length; i<len; i++){
            ...
            headersArr = headersArr.concat([].concat(headers[key]).map((function(field){
                return mimelib.foldLine(key+": "+this._sanitizeHeaderValue(field, key), 76, false, false, 52);
            }).bind(this)));
        }
    }
 ...
 */
MailComposer.prototype._sanitizeHeaderValue = function(value, key){
