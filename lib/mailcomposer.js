...
MailComposer.prototype._createAttachmentComponent = function(attachment){
    ...
    node.contentEncoding = attachment.contentEncoding || "base64";
...
MailComposer.prototype._emitDataElement = function(element, callback){
    ...
    if(element.filePath){
        if(element.filePath.match(/^https?:\/\//)){
            this._serveStream(urlFetch(element.filePath, {userAgent: element.userAgent}),
                element.contentEncoding, callback);
        }else{
            this._serveFile(element.filePath, element.contentEncoding, callback);
        }
        return;
    }else if(element.streamSource){
        this._serveStream(element.streamSource, element.contentEncoding,
            callback);
        return;
    }
 ...
 */
MailComposer.prototype._serveFile = function(filePath, encoding, callback){
    fs.stat(filePath, (function(err, stat){
        ...
        this._serveStream(stream, encoding, callback);
