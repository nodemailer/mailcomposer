...
MailComposer.prototype.generateBodyStructure = function(){
    // TODO: lõpetada
    var tree = this.createMimeNode(), 
        currentNode, node,
        i, len;
    
    if(this._message.useMixed){
        
        node = this.createMimeNode();
    ...
    if(this._message.body){
        node = this.createTextComponent(this._message.body, "text/plain");
        if(currentNode){
            currentNode.childNodes.push(node);
            node.parentNode = currentNode;
        }else{
    ...
    if(this._message.html){
        node = this.createTextComponent(this._message.html, "text/html");
        if(currentNode){
            currentNode.childNodes.push(node);
            node.parentNode = currentNode;
        }else{
    ...
    if(this._attachments){
        for(i=0, len = this._attachments.length; i<len; i++){
            node = this.createAttachmentComponent(this._attachments[i]);
            node.parentNode = currentNode;
            currentNode.childNodes.push(node);
        }
    }
 ...
 */
MailComposer.prototype.createTextComponent = function(text, contentType){
    var node = this.createMimeNode();
    ...
    if(["7bit", "8bit", "binary"].indexOf(node.encoding)>=0){
 ...
 * @return {Object} Mime tree node 
 */
MailComposer.prototype.createAttachmentComponent = function(attachment){
    var node = this.createMimeNode(),
    ...
    if(attachment.fileName){
        fileName = this.encodeMimeWord(attachment.fileName, "Q", 1024).replace(/"/g,"\\\"");
    ...
    if(attachment.cid){
        node.headers.push(["Content-Id", "<" + this.encodeMimeWord(attachment.cid) + ">"]);
    }
 ...
 * @return {Object} Mime tree node
 */
MailComposer.prototype.createMimeNode = function(){
    return {
        childNodes: [],
        headers: [],
        parentNode: null
 ...
 */
MailComposer.prototype.flattenMimeTree = function(){
    var flatTree = [];
    ...
    for(len = flatTree.length; this._message.processingPos < len; this._message.processingPos++){
        ...
        if(isEnd || isObject){
            ...
            if(isObject){
                ...
                this.emitDataElement(curObject, (function(){
                    if(!isEnd){
                        process.nextTick(this.composeBody.bind(this));
                    }else{
                        this.emit("end");
                    }
                }).bind(this));
 ...
 */
MailComposer.prototype.emitDataElement = function(element, callback){
    ...
    if(element.filePath){
        this.serveFile(element.filePath, callback);
        return;
    }
 ...
 */
MailComposer.prototype.serveFile = function(filePath, callback){
