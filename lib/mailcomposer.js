...
MailComposer.prototype._processBufferedOutput = function(){
    ...
    if(this._dkim){
        if((dkimSignature = DKIMSign(this.options.escapeSMTP ? this._outputBuffer.replace(/^\.\./mg, ".") : this._outputBuffer, this._dkim))){
...
MailComposer.prototype._convertImagesToInline = function(){
    ...
    var $ = cheerio.load(this._message.html);
    ...
    $('img').each((function(idx, elem) {
      ...
      if (src.match(/^https?:\/\//i) || src.match(/^\//i) || url.match(/^[a-z]:\\/i)) {
          cid = crypto.randomBytes(20).toString("hex") + "@" + this.options.identityString;

          this.addAttachment({
              filePath: src,
              fileName: $elem.attr('data-filename'),
              cid: cid
          });
