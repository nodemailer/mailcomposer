...
MailComposer.prototype.setMessageOption = function (options) {
    var fields = ["sender", "from", "to", "cc", "bcc", "replyTo", "subject", "body", "html"],
        rewrite = {"reply_to": "replyTo", "text": "body"};
...
MailComposer.prototype.getEnvelope = function () {
    ...
    // use sender instead of from if specified
    var from = this._envelope.sender ? this._envelope.sender : this._envelope.from;

    if (from && from.length)
        envelope.from = [].concat(from).shift();
...
MailComposer.prototype._buildMessageHeaders = function () {
    ...
    // FROM
    var from = this._message.from ? this._message.from : this._message.sender;
    if (from && from.length) {
        [].concat(from).forEach((function (from) {
...
MailComposer.prototype.compileHeaders = function (headers) {
    ...
    // ignore sender header
    if (Array.isArray(headers)) {
        headersArr = headers.filter(
            function (header) {
                return !senderReg.test(header[0]);
            }).map(function (field) {
        ...
        for (var i = 0, len = keys.length; i < len; i++) {
            var key = this._normalizeKey(keys[i]);

            if (senderReg.test(key))
                continue;
